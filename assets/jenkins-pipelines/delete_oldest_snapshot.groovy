        ...
        stage('Delete') {
            steps {
                    script {
                        ...
                        SNAPSHOTID=\$(aws fsx describe-snapshots --filters "Name=volume-id,Values=$FSX_WORKSPACE_VOLUME_ID" --query "sort_by(Snapshots,&CreationTime)[?CreationTime<='$(date +%Y-%m-%dT23:59:59.999999+23:59 -d "7 days ago")' && Lifecycle == 'AVAILABLE']" --output json | jq -r '.[0].SnapshotId')

...
8. **Configuration Script**: `/home/ubuntu/swarm_scripts/swarm_configure.sh` for runtime setup (see Runtime Configuration Details below)
...
The following are configured at **runtime** when you launch an instance:
...
3. **Swarm Configuration**: Executes `/home/ubuntu/swarm_scripts/swarm_configure.sh` which:
...
==> amazon-ebs.ubuntu2404: Creating AMI p4_code_review_ubuntu2404-20231209123456 from instance i-xxxxx
==> amazon-ebs.ubuntu2404: AMI: ami-0abc123def456789
...
aws ec2 describe-images \
  --owners self \
  --filters "Name=name,Values=p4_code_review_ubuntu2404-*" \
...
|                         DescribeImages                                |
+----------------------+---------------------------------------+--------+
|  ami-0abc123def456   | p4_code_review_ubuntu2404-20231209    | 2023...|
|  ami-0def456abc789   | p4_code_review_ubuntu2404-20231208    | 2023...|
+----------------------+---------------------------------------+--------+
```
...
1. **Navigate to the Terraform module**:
   ...
   ```bash
   cd ../../../../modules/perforce/modules/p4-code-review
   ```
...
3. **Follow the deployment guide** in the module README, which covers:
   - Creating AWS Secrets Manager secrets for P4 credentials
   - Writing Terraform configuration
   - Deploying the infrastructure
   - Accessing the Swarm web console

Or jump directly to: [P4 Code Review Module Documentation](../../../../modules/perforce/modules/p4-code-review/README.md)
...
**Error**: "AMI name 'p4_code_review_ubuntu2404-TIMESTAMP' already exists"

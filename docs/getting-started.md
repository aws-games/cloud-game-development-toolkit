...
Configurations for the _Simple Build Pipeline_ are split between 2 files: [`local.tf`] and [`variables.tf`]. Variables in [`local.tf`] are typically static and can be modified within the file itself. Variables in [`variables.tf`], tend to be more dynamic and are passed in through the `terraform apply` command either directly through a `-var` flag or as file using the `-var-file` flag.

...
Two sets of secrets, stored in Amazon Secrets Manager, are required to properly deploy this module.
The first is a `external-idp-oidc-credentials` secret which is made up of the `client-id` and `client-secret` used to configure your IDP provider. The second is a `github_credential` which is made up of your `github-username` and `github-access-token` which are used to access the necessary container images for deploying Unreal Cloud DDC.

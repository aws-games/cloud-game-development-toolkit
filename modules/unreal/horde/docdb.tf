...
resource "aws_docdb_cluster_parameter_group" "horde" {
  ...
  #checkov:skip=CKV_AWS_90:TLS disabled until public cert can be downloaded to Horde service
...
resource "aws_docdb_cluster_instance" "horde" {
  ...
  cluster_identifier = aws_docdb_cluster.horde.id
...
resource "aws_docdb_cluster" "horde" {
  ...
  db_cluster_parameter_group_name = aws_docdb_cluster_parameter_group.horde.name
  db_subnet_group_name            = aws_docdb_subnet_group.horde.name
  vpc_security_group_ids          = [aws_security_group.unreal_horde_docdb_sg.id]

...
| <a name="input_agents"></a> [agents](#input\_agents) | Configures autoscaling groups to be used as build agents by Unreal Engine Horde. | <pre>map(object({<br>    ami           = string<br>    instance_type = string<br>    min_size      = optional(number, 0)<br>    max_size      = optional(number, 1)<br>  }))</pre> | `{}` | no |

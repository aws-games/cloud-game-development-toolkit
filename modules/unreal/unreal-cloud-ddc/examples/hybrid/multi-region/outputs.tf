...
output "us-east-1" {
  ...
  value = {
    ...
    ddc_endpoint = module.unreal_cloud_ddc_primary.ddc_connection.endpoint
    # NLB endpoint removed - LoadBalancer service creates NLB dynamically
    bearer_token_secret_arn = module.unreal_cloud_ddc_primary.bearer_token_secret_arn
    # Security warning removed - not available in current module version

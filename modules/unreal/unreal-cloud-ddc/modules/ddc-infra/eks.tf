...
resource "null_resource" "aws_load_balancer_controller_crds" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      set -e
      echo "[CRD] Installing AWS Load Balancer Controller CRDs..."
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      echo "[CRD CLEANUP] SUCCESS: Cleanup completed"
    EOT
  }
...
resource "null_resource" "aws_load_balancer_controller" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      set -e
      echo "[ALB-CONTROLLER] Installing AWS Load Balancer Controller with IRSA..."
      ...
      echo "[ALB-CONTROLLER] Restarting controller to ensure fresh IAM permissions..."
      kubectl rollout restart deployment/aws-load-balancer-controller -n kube-system
      kubectl rollout status deployment/aws-load-balancer-controller -n kube-system --timeout=120s

      echo "[ALB-CONTROLLER] SUCCESS: AWS Load Balancer Controller installation completed"
    EOT
  }
  ...
  provisioner "local-exec" {
    when = destroy
    command = <<-EOT
      echo "[ALB-CONTROLLER CLEANUP] Starting cleanup..."

      # Configure kubectl (ignore failures if cluster deleted)
      aws eks update-kubeconfig --name ${self.triggers.cluster_name} --region ${self.triggers.region} || {
        echo "[ALB-CONTROLLER CLEANUP] Cluster already deleted, skipping cleanup"
        exit 0
      }
      ...
      echo "[ALB-CONTROLLER CLEANUP] SUCCESS: Cleanup completed"
    EOT
  }
...
resource "null_resource" "cert_manager" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      set -e
      echo "[CERT-MANAGER] Installing Cert Manager with IRSA..."
      ...
      echo "[CERT-MANAGER] SUCCESS: Cert Manager installation completed"
    EOT
  }
  ...
  provisioner "local-exec" {
    when = destroy
    command = <<-EOT
      echo "[CERT-MANAGER CLEANUP] Starting cleanup..."

      # Configure kubectl (ignore failures if cluster deleted)
      aws eks update-kubeconfig --name ${self.triggers.cluster_name} --region ${self.triggers.region} || {
        echo "[CERT-MANAGER CLEANUP] Cluster already deleted, skipping cleanup"
        exit 0
      }
      ...
      echo "[CERT-MANAGER CLEANUP] SUCCESS: Cleanup completed"
    EOT
  }
...
resource "null_resource" "ddc_nodepool" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      set -e
      echo "[NODEPOOL] Creating DDC NodePool for i-family instances with NVMe storage..."
      ...
      echo "[NODEPOOL] Waiting for IAM policy propagation..."
      sleep 30
      
      # Verify cluster role has custom tagging policy
      echo "[NODEPOOL] Verifying cluster role IAM permissions..."
      ...
      echo "[NODEPOOL] Deleting existing NodeClass to force recreation..."
      kubectl delete nodeclass ddc-nodeclass --ignore-not-found=true
      
      # Create NodeClass for DDC workloads (NVMe optimized)
      echo "[NODEPOOL] Creating NodeClass with IAM role: ${aws_iam_role.eks_node_role.name}"
...
EOF
      ...
      echo "[NODEPOOL] NodeClass created successfully, creating NodePool..."
    ...
    spec:
      ...
      requirements:
        - key: karpenter.sh/capacity-type
          operator: In
          values: ["on-demand"]
        # Target i-family instances with NVMe storage
...
EOF
      ...
      echo "[NODEPOOL] ✅ DDC NodePool created successfully"
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      echo "[NODEPOOL CLEANUP] ✅ Cleanup completed"
    EOT
  }

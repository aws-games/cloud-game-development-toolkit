...
# References our Terraform-managed EKS cluster security group
resource "aws_vpc_security_group_egress_rule" "nlb_to_cluster" {
  ...
  from_port                    = 80
  to_port                      = 8091
  referenced_security_group_id = module.ddc_infra.cluster_security_group_id
...
# EKS cluster accepts traffic from NLB (corresponding ingress rule)
resource "aws_vpc_security_group_ingress_rule" "cluster_from_nlb" {
  ...
  description                  = "Allow NLB traffic to DDC services"
  ip_protocol                  = "tcp"
  from_port                    = 80
  to_port                      = 8091

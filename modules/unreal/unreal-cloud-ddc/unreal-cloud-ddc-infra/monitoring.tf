...
resource "aws_lb" "scylla_monitoring_alb" {
  ...
  security_groups                  = length(var.existing_security_groups) > 0 ? concat([aws_security_group.scylla_monitoring_lb_sg[count.index].id], var.existing_security_groups) : [aws_security_group.scylla_monitoring_lb_sg[count.index].id]

...
| <a name="input_build_farm_config"></a> [build\_farm\_config](#input\_build\_farm\_config) | Build agent configurations. Each agent can have custom environment variables via the 'environment' map. | <pre>map(object({<br/>    image         = string<br/>    desired_count = number<br/>    cpu           = number<br/>    memory        = number<br/>    environment   = optional(map(string), {})<br/>  }))</pre> | `{}` | no |

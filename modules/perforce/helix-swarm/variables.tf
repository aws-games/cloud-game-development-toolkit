...
variable "create_external_alb" {
  type        = bool
  default     = true
  description = "This flag controls the creation of an external ALB for the Helix Swarm service."
}

variable "create_internal_alb" {
  type        = bool
  default     = true
  description = "This flag controls the creation of an internal ALB for the Helix Swarm service."
}

variable "helix_swarm_external_alb_subnets" {
  type        = list(string)
  description = "A list of subnets to deploy the Helix Swarm external load balancer into. Public subnets are recommended."
  validation {
    condition     = var.create_external_alb && length(var.helix_swarm_external_alb_subnets) > 0
    error_message = "You must specify subnets for the external ALB."
  }
}

variable "helix_swarm_internal_alb_subnets" {
  type        = list(string)
  description = "A list of subnets to deploy the Helix Swarm internal load balancer into. Private subnets are recommended."
  validation {
    condition     = var.create_internal_alb && length(var.helix_swarm_internal_alb_subnets) > 0
    error_message = "You must specify subnets for the internal ALB."
  }
}

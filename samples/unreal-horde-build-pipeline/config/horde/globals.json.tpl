{
  "_comment": [
    ...
    "    cluster with NO credentials and fails. So we DO define it here.",
    ...
    "  - agentTypes keys map to BuildGraph <Agent> elements by their Type attribute (e.g. AnyAgent)",
    "    and/or the platform (Win64), in addition to named agents. The four entries cover both:",
    "    the named agents \"SyncAgent\"/\"BuildAgent\" (matching <Agent Name=\"...\"> in the XML), and",
    "    the Type/platform-based mapping - AnyAgent -> sync-pool",
    "    (the BuildGraph <Agent Type=\"AnyAgent\"> attribute) and Win64 -> build-pool (the platform",
    "    agentType). SyncAgent uses type=SyncPool and BuildAgent type=BuildPool.",
    ...
    "  - BOTH POOLS ARE WINDOWS, because the move to iSCSI requires it: the",
    ...
    "      * SnapshotName        - which snapshot to clone; picked per build (highest",
    ...
    "",
    "SCHEMA AND COUPLING FACTS:",
    "  1. Pool condition expression syntax is an instance-tag match - the pools",
    "     below select by instance tag (\"aws-tag == 'Horde_Autoscale_Pool:<Pool>'\").",
    "  2. The server reads this flat top-level (version 1) schema.",
    "  3. -Script= paths (Build/HydratePipeline.xml, Build/BuildPipeline.xml) resolve",
    "     against the stream root once the buildgraph/ files are submitted to depot under Build/.",
    "  4. -Target node names (\"Sync And Snapshot\", \"Compile\") match the node/aggregate names",
    "     defined in the BuildGraph XML."
  ],
  "version": 1,
  "perforceClusters": [
    {
  ...
  "_comment_storage": [
    ...
    "backend (requires a dedicated bucket + task-role IAM permissions)."
  ],
  "storage": {
    "backends": [

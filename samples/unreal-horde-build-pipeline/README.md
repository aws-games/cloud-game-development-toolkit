...
6. **Agent P4 SSL trust.** The Perforce endpoint is SSL (`ssl:<host>:1666`). Before an agent can run `p4` commands against it, the agent's OS user must trust the server's SSL fingerprint (`p4 trust -y`) and then authenticate (`p4 login`). A fresh agent that has never trusted the server will fail its first `p4` command with an SSL trust error until `p4 trust` is run for that user. Ensure the agent's P4 user establishes trust and logs in (typically as part of agent bootstrap) before the sync/build steps run.

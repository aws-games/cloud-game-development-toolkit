...
locals {
  # EMPTY ON THE SAN PATH. This map used to carry the Linux Ansible playbook that
  # AWS-ApplyAnsiblePlaybooks pulled from S3 to fstab-mount the source volume over
  # NFSv3. The hydrator is Windows now (the source LUN carries NTFS), so both pools
  # are configured inline via AWS-RunPowerShellScript and nothing needs staging in
  # S3. The bucket itself is kept: the agent instance role's policy and the
  # agent_config_bucket output both reference it, and it remains the right place to
  # stage larger agent payloads.
  agent_config_files = {}
}
...
# --- Hydrator (Sync) Agent SSM Association -----------------------------------
#
# WINDOWS, via AWS-RunPowerShellScript. Previously Linux, via
# AWS-ApplyAnsiblePlaybooks pulling config/sync-agent.ansible.yml from S3.
#
# The change is forced by the data path, not chosen: the source LUN carries NTFS,
# so its writer must be Windows. Both agent pools therefore share one setup
# template, distinguished only by the agent_role interpolation.
resource "aws_ssm_association" "configure_sync_agent" {
  ...
  parameters = {
    commands = templatefile("${path.module}/config/agent-san-setup.ps1.tpl", {
      agent_role = "hydrator"
